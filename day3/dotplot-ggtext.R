...
dates <- tibble::tribble(
  ...
  transmute(
    ...
    age_at_coronation = (coronation - birth) |>
      as.duration() |>
      as.numeric("years") |>
      floor(),
...
surv_coronation <- dates |>
  ...
  mutate(lx_coronation = lx, prop_surv_coronation = lx / 1e5)
...
comb_peers |>
  ...
  geom_text(
    ...
    aes(
      x = longevity,
      label = prop_outlived |> multiply_by(1e2) |> round(1) |> paste0("%")
    ),
    hjust = 1.2,
    size = 3.2,
    family = font_rc,
